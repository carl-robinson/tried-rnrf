...
def showmap(sm, sztext=11, colbar=True, cmap=cm.jet, interp=None,caxmin=None,
            caxmax=None,axis=None) :
    ...
    fig = plt.figure();  
    nbsubc = np.ceil(np.sqrt(sm.dim));
    nbsubl = np.ceil(sm.dim/nbsubc);
    isub=0;
    for i in np.arange(sm.dim) :
        isub+=1;
...
def showmapping(sm, Data, bmus, seecellid=1, subp=1, override=False,
    ...
    #Data = sm.data; # On pourrait vouloir voir avec d'autres
                     # données (de Test pas exemple) ?
        ...
        for j in (np.arange(sm.dim-i-1)+i+1) :      
            ...
            for k in np.arange(sm.nnodes) :
                ...
                if seecellid==True : # Ajouter un indice des neurones (mais est-ce le bon ?)
                    ...
                    #plt.text(Ref2D[k,0],Ref2D[k,1], k+1, fontsize=20,color='r');
                if bmus!=[] and bmus is not None : # Si Les BMUS sont passés ca signifie
...
def showcarte (sm, figlarg=8,fighaut=6,hits=None, shape='o', shapescale=300, \
    ...
    #fig = plt.figure(figsize=(8,6)); # taille std par defaut
    fig = plt.figure(figsize=(figlarg,fighaut)); # taille std par defaut
    #shapescale = 300; # 351 : Echelle qui semble bien aller avec
    #                  # la taille (8,6) de la figure par defaut
    delta      = 1/Mmsz;
    midelta    = delta/2;
    left       = (1 - delta*mmsz)/2;
    #print("delta=",delta," midelta=",midelta," left=",left)
    ...
    if hits!=[] and hits is not None :
        if np.size(hits,0) != sm.nnodes :
            ...
            sys.exit(0);
        hitmax = np.max(hits);
        hitdscale  = delta*shapescale/hitmax; #print("hitdscale=",hitdscale);
        WithHits = True;
    else :
...
def showbarcell (sm,norm='brute',a=0,b=1,scale=0,cmap=cm.rainbow,sztext=11) :
    ...
    #Tcol = cmap(np.arange(1,256,round(256/sm.dim).astype(int)));
...
def showprofils(sm, visu=1, Data=None, bmus=None, scale=None, \
    ...
    if Clevel == [] or Clevel is None :
        ...
        if Gscale<0 or Gscale>1 :
            ...
            sys.exit(-1);
        minx=min(Clevel); maxx=max(Clevel); dx=maxx-minx; dr=b-Gscale;
...
def showrefpat(sm,x,x0,x1, bmus=None ,Data=None, hits=None,
    ...
    for k in np.arange(sm.nnodes) :
        ...
        #plt.title("%d-%d" %(k+1, hits[k]),fontsize=sztext,color=coltext); # c'était tagué ; why ?
        if titlepos is None :
    ...
    #plt.suptitle("Summation of pattern by units who captured the corresponding data")
...
def cblabfreq(Tfreq,Ulab,csep=' ') :
    ...
    '''
    nnodes = len(Tfreq); nlabels=len(Ulab)
    CBLABELS = [];
    for i in np.arange(nnodes) :
        strlabi=''
...
def confus(sm,Data,Labels,classnames,CBlabels=None,Databmus=None,visu=False) :
    ...
    nmclasses   = list(classnames); # Copy pour préserver classname car j'ai vu
                   # que ctk.label2ind ajoute '' devant ...
