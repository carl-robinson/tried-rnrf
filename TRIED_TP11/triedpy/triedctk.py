    ...
    for i in comp :
        ...
        if nodes is not None :
            for n in np.arange(len(Unodes)) :
                lig  = Unodes[n] // nbc; #print(n, lig)
                col  = Unodes[n] % nbc;  #print(n, col)
                slen = len(Ulabels[n]);
...
def showrefpat(sm,x,x0,x1, bmus=None ,Data=None, hits=None,
    ...
    for i in np.arange(nlx) :
        asdf_col = SPR[:,bmus[i]]
        asdf_x = x[i,:]
        asdf_x = np.array(asdf_x)
        asdf_x = asdf_x.reshape((np.size(asdf_x),1))
        asdf_sum = asdf_col + asdf_x
        SPR[:,bmus[i]] = asdf_sum
