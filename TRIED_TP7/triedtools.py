...
def centred (X,meanx=None,stdx=None,biais=1,coef=1) :
    ''' Xcr = .centred(X,biais);
    | Retourne Xcr qui correspond au centrage et à la réduction des colonnes de X.
...
def classperf(Yref, Yest, crit="max", miss=0, idgoodbad=0) :
    ...
    | max de 2 vecteurs (ligne) ont sur le même indice quelque soit l'écart
    | entre ces 2 max. Actuelement c'est le seul critère utilisé.
...
def shownpat2D(x,xshape,fr,to,subp=True,cmap=cm.jet) :
